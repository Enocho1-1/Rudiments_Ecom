...
export const Checkout = () => {
    ...
    const orderPrice = discountPrice === null ? total : discountPrice
  return (
    <div className="relative max-w-inherit mt-8 py-2 max-mobile:px-4">
        ...
        <span className="flex flex-col text-xl max-mobile:text-lg">
            ...
            <p className={`ml-[20rem] max-mobile:ml-[9.375rem] text-lg ${discountPrice > 0 ? (`text-red-500`) : (`text-black`)}`}>US${orderPrice}.00</p>
            <p className={`text-xl font-bold ml-[20rem] max-mobile:ml-[9.375rem] max-mobile:text-lg ${discountPrice > 0 ? (`text-red-500`) : (`text-black`)}`}>US${orderPrice}.00</p>
            </span>
        </aside>
