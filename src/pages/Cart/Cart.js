...
export const Cart = ({name}) => {
  ...
  const handlePromo = (e) => {
    ...
    // Discount Cart Total
    discountUserTotal(total,userPromo,promoCode,setDiscountPrice, setPromoApplied,  setPromoError, discountPriceStore)
    e.target.reset()
  }
        ...
        <aside className="relative m-auto max-w-7xl mt-24 flex flex-col">
         <Link to="/" className="mt-4 px-4 flex hover:text-slate-500">
            ...
            <h1 className="text-xl  font-semibold mx-2 ">Continue Shopping</h1>
               ...
               {promoApplied && (<div className="fixed top-[20%] right-4 max-mobile:relative max-mobile:top-5 max-mobile:left-[23%] py-2 px-2 flex rounded-md border-2 border-green-400 text-green-400 max-w-[15.625rem]"><span className="text-4xl bi bi-check2-circle"></span><p className="ml-2 text-md font-semibold self-center">20% Promo Applied ‼️</p></div>)}
                ...
                <aside className="flex">
                  <span className="flex flex-col text-xl max-mobile:text-lg">
                    ...
                    <span className="flex flex-col">
                      <p className={`ml-[20rem] max-mobile:ml-[9.375rem] text-lg ${discountPrice > 0 ? (`text-red-500`) : (`text-black`)}`}>US${discountPrice === 0 ? total : discountPrice}.00</p>
                      <p className={`text-xl font-bold ml-[20rem] max-mobile:ml-[9.375rem] max-mobile:text-lg ${discountPrice > 0 ? (`text-red-500`) : (`text-black`)}`}>US${discountPrice === 0 ? total : discountPrice}.00</p>
                    </span>
                </aside>
